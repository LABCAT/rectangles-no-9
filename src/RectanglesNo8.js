...
const audio = new URL("@audio/rectangles-no-8.ogg", import.meta.url).href;
const midi = new URL("@audio/rectangles-no-8.mid", import.meta.url).href;
    ...
    p.draw = () => {
        if(p.audioLoaded && p.song.isPlaying() || p.songHasFinished){
            ...
            for (let particle = 0; particle < p.particles.length; particle++) {
                ...
                p.rect(p.positionX[particle], p.positionY[particle], p.particles[particle].size * 2000, p.particles[particle].size * 2000);
                p.stroke(0, 0, 100, strokeOpacity);
                p.fill(hue2, 100, 100, 0.25);
                p.rect(p.positionX[particle], p.positionY[particle], p.particles[particle].size * 1000, p.particles[particle].size * 1000);
                p.stroke(hue3, 100, 100, 1);
                p.fill(hue3, 100, 100, 0.25);
                p.rect(p.positionX[particle], p.positionY[particle], p.particles[particle].size * 500, p.particles[particle].size * 500);
                p.stroke(hue3, 100, 100, 0);
                p.fill(hue4, 100, 100, 0.25);
                p.rect(p.positionX[particle], p.positionY[particle], p.particles[particle].size * 250, p.particles[particle].size * 250);
            }
        }
    }
